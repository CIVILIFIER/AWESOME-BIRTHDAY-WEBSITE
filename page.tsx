...
export default function Page() {
  ...
  const makeWish = () => {
    const customWish = wishText.trim();
    if (!customWish || !wishSealed || wishSent) return;

    setWishSent(true);

    // One strong burst at the moment the sealed wish is sent, followed by
    // smaller warm bursts so the transition feels like fire opening the sky.
    spawnFirework([0, 0.2, -7], "#fff0b8", 1.9);
    window.setTimeout(() => spawnFirework([-2.8, 1.4, -8], "#ff9c5a", 1.15), 90);
    window.setTimeout(() => spawnFirework([2.8, 1.2, -8], "#ffce72", 1.1), 180);
    window.setTimeout(() => spawnFirework([-2.1, -1.2, -8], "#ff6f91", 0.95), 300);
    window.setTimeout(() => spawnFirework([2.2, -1.15, -8], "#ff8fdd", 0.95), 430);
    window.setTimeout(() => spawnFirework([0, 0.1, -6.5], "#ffffff", 1.25), 560);
  };
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      memories.forEach((slide) => {
        ...
        const bg = slide.querySelector<HTMLElement>('.memory-bg-image');
        if (!stage || !image || !card || !copy || !bg) return;
        ...
        // Full-bleed memory image: enters from the edge, settles behind the card,
        // then drifts away as the next memory approaches.
        const memoryIndex = Number(slide.dataset.memory || '1') - 1;
        const bgDirection = memoryIndex % 2 === 0 ? 1 : -1;
        const bgEnter = easeOut(THREE.MathUtils.clamp(progress / 0.30, 0, 1));
        const bgExit = easeOut(THREE.MathUtils.clamp((progress - 0.67) / 0.33, 0, 1));
        const bgX = bgDirection * (12 - bgEnter * 12 - bgExit * 18);
        const bgY = 46 - bgEnter * 46 - bgExit * 34;
        const bgScale = 1.12 - focus * 0.12 + bgExit * 0.08;
        const bgOpacity = THREE.MathUtils.clamp(0.01 + focus * 0.17 - bgExit * 0.12, 0.01, 0.18);
        const bgBlur = 8 - focus * 6 + bgExit * 5;
        ...
        bg.style.setProperty('--memory-bg-x', `${bgX.toFixed(1)}px`);
        bg.style.setProperty('--memory-bg-y', `${bgY.toFixed(1)}px`);
        bg.style.setProperty('--memory-bg-scale', bgScale.toFixed(4));
        bg.style.setProperty('--memory-bg-opacity', bgOpacity.toFixed(3));
        bg.style.setProperty('--memory-bg-blur', `${bgBlur.toFixed(1)}px`);
  ...
  useEffect(() => {
    ...
    const revealGroups: Array<{ selector: string; variant: string }> = [
      ...
      { selector: '.interactive-wish-slide .mini-label, .interactive-wish-slide h2, .interactive-wish-slide .wish-choice-copy, .interactive-wish-slide .firework-button', variant: 'float' },
      ...
      { selector: '.support-slide .support-wish', variant: 'lines' },
  ...
  /* =======================================================
     IGLOO-STYLE CINEMATIC SCROLL MOTION
     One RAF drives scale-through transitions, depth drift,
     rotation, blur and section spotlighting.
  ======================================================= */
  useEffect(() => {
    if (!introComplete) return;
    ...
    const update = () => {
      ...
      sections.forEach((section) => {
        ...
        const rawDistance = Math.abs(sectionCenter - center) / (viewport * 0.9);
        const focus = 1 - clamp(rawDistance, 0, 1);
        const eased = smooth(focus);
        const direction = sectionCenter < center ? -1 : 1;

        const scale = 0.90 + eased * 0.10;
        const y = direction * (1 - eased) * 36;
        const rotate = direction * (1 - eased) * 1.5;
        const blur = (1 - eased) * 2.4;
        const opacity = 0.74 + eased * 0.26;
        const depth = (1 - eased) * 34;

        section.style.setProperty("--igloo-scale", scale.toFixed(4));
        section.style.setProperty("--igloo-y", `${y.toFixed(1)}px`);
        section.style.setProperty("--igloo-rotate", `${rotate.toFixed(2)}deg`);
        section.style.setProperty("--igloo-blur", `${blur.toFixed(2)}px`);
        section.style.setProperty("--igloo-opacity", opacity.toFixed(3));
        section.style.setProperty("--igloo-focus", eased.toFixed(3));
        section.style.setProperty("--igloo-depth", `${depth.toFixed(1)}px`);

        section.querySelector<HTMLElement>(".slide-inner, .memory-sequence, .maki-header, .letter-header, .cake-stage, .maki-reveal, .letter-paper-wrap")?.style.setProperty("--igloo-inner-y", `${(-direction * (1 - eased) * 22).toFixed(1)}px`);
        section.querySelector<HTMLElement>(".memory-stage, .cake-sky-celebration, .maki-reveal, .letter-paper-wrap")?.style.setProperty("--igloo-scale-inner", (0.94 + eased * 0.06).toFixed(4));
      });
    };
  ...
  return (
    <main className="page">
      ...
      {!introComplete && (
        <section className="intro grand-intro">
          ...
          <div className="intro-content grand-intro-content">
            ...
            <div className="birthday-wordmark advanced-birthday-wordmark" aria-label="Happy Birthday Uzma">
              <span>THE DAY</span>
              <span>IS YOURS</span>
            </div>

            <div className="intro-age-wrap advanced-age-wrap">
              <div className="intro-age">19</div>
              <div className="intro-age-label">A NEW CHAPTER</div>
            </div>
            ...
            <p className="intro-small advanced-intro-small">
              Some birthdays deserve more than a message.
      ...
      >
        ...
        <section className={`slide interactive-wish-slide ${wishSent ? "wish-sent" : ""}`}>
          {!wishSent ? (
            <div className="wish-launch-panel">
              ...
              <div className="wish-choice-dock" role="group" aria-label="Choose a wish">
                <button type="button" className={`wish-choice ${selectedWish === "happiness" ? "selected" : ""}`} aria-pressed={selectedWish === "happiness"} onClick={() => setSelectedWish("happiness")}>
                  <span>01</span> HAPPINESS
                </button>
                <button type="button" className={`wish-choice ${selectedWish === "memories" ? "selected" : ""}`} aria-pressed={selectedWish === "memories"} onClick={() => setSelectedWish("memories")}>
                  <span>02</span> MEMORIES
                </button>
                <button type="button" className={`wish-choice ${selectedWish === "dreams" ? "selected" : ""}`} aria-pressed={selectedWish === "dreams"} onClick={() => setSelectedWish("dreams")}>
                  <span>03</span> DREAMS
                </button>
              </div>
                ...
                {!wishSealed ? (
                  <>
                    ...
                    <div className="wish-capsule-row">
                      <input value={wishText} onChange={(event) => setWishText(event.target.value.slice(0, 72))} placeholder="type a secret wish…" aria-label="Write a secret wish" />
                      <button type="button" className="wish-seal-button" onClick={() => setWishSealed(true)} disabled={!wishText.trim()}>
                        SEAL IT
                      </button>
                    </div>
                  </>
              ...
              <button type="button" className={`firework-button ${wishSealed ? "wish-ready-button" : ""}`} onClick={makeWish} disabled={!wishSealed}>
                {wishSealed ? "✦ MAKE MY WISH ✦" : "✦ SEAL YOUR WISH FIRST ✦"}
              </button>
            </div>
          ) : (
            <div className="wish-sent-panel" aria-live="polite">
              <div className="wish-fire-orbit" aria-hidden="true">
                {Array.from({ length: 18 }, (_, i) => (
                  <span key={i} style={{ ["--fire-angle" as string]: `${(360 / 18) * i}deg`, ["--fire-delay" as string]: `${(i % 6) * 0.08}s` } as React.CSSProperties} />
                ))}
                <i className="wish-fire-core" />
              </div>

              <div className="wish-sent-kicker">✦ WISH RECEIVED ✦</div>
              <div className="wish-sent-seal">SEALED FOR THE UNIVERSE</div>
              <div className="wish-sent-message">“{wishText.trim()}”</div>

              <div className="wish-my-wish">
                <span>MY WISH</span>
                <strong>{selectedWish === "happiness" ? "MORE HAPPINESS, ALWAYS." : selectedWish === "memories" ? "MORE MOMENTS WORTH KEEPING." : "MORE DREAMS WORTH CHASING."}</strong>
              </div>

              <div className="wish-sent-foot">May it find you at exactly the right time.</div>
            </div>
          )}
        </section>
        ...
        <section className="slide support-slide">
          <div className="support-bg-word" aria-hidden="true">WISHES</div>
          ...
          <div className="support-container wish-wall">
            ...
            <div className="support-heading-row">
              ...
              <div className="support-meta">06 THINGS<br />I'D WISH FOR YOU</div>
            </div>
            <p className="support-intro">Not the usual birthday-list kind. These are the little things I genuinely hope find their way into your 19th year.</p>

            <div className="support-lines">
              <article className="support-wish">
                <div className="support-wish-index">01</div>
                <div className="support-wish-copy">
                  <span>SOFT MORNINGS</span>
                  <p>I wish your days begin gently — with good moods, good coffee, and at least one reason to smile before noon.</p>
                </div>
                <div className="support-wish-mark">↗</div>
              </article>

              <article className="support-wish">
                <div className="support-wish-index">02</div>
                <div className="support-wish-copy">
                  <span>LOUD LAUGHS</span>
                  <p>I wish you those stupid, uncontrollable laughs that make your stomach hurt and somehow become the memories you keep forever.</p>
                </div>
                <div className="support-wish-mark">↗</div>
              </article>

              <article className="support-wish">
                <div className="support-wish-index">03</div>
                <div className="support-wish-copy">
                  <span>UNEXPECTED MAGIC</span>
                  <p>I wish random plans turn into favourite stories, ordinary days become unexpectedly beautiful, and surprises find you when you least expect them.</p>
                </div>
                <div className="support-wish-mark">↗</div>
              </article>

              <article className="support-wish">
                <div className="support-wish-index">04</div>
                <div className="support-wish-copy">
                  <span>YOUR PEOPLE</span>
                  <p>I wish you stay surrounded by people who make you feel safe enough to be completely yourself and silly enough to laugh at absolutely nothing.</p>
                </div>
                <div className="support-wish-mark">↗</div>
              </article>

              <article className="support-wish">
                <div className="support-wish-index">05</div>
                <div className="support-wish-copy">
                  <span>BIG DREAMS</span>
                  <p>I wish the things you quietly want for yourself stop feeling distant — and that this year gives you a few moments where you realize, “wait… I actually did it.”</p>
                </div>
                <div className="support-wish-mark">↗</div>
              </article>

              <article className="support-wish support-wish-final">
                <div className="support-wish-index">06</div>
                <div className="support-wish-copy">
                  <span>ONE SELFISH WISH</span>
                  <p>And maybe a little selfishly… I wish somewhere in this next chapter, I get to be part of a few of your favourite memories too. ♡</p>
                </div>
                <div className="support-wish-mark">✦</div>
              </article>
            </div>

            <div className="support-endnote">SAVE THE GOOD DAYS. KEEP THE CHAOS. MAKE IT YOUR YEAR.</div>
          </div>
        </section>
        ...
        <section className="slide final-slide">
          ...
          <div className="final-spark-field" aria-hidden="true">
            {Array.from({ length: 26 }, (_, i) => (
              <i key={i} style={{
                '--sx': `${seededRandom(i, 701) * 100}%`,
                '--sy': `${seededRandom(i, 702) * 100}%`,
                '--sd': `${seededRandom(i, 703) * 2.5}s`,
                '--ss': `${0.55 + seededRandom(i, 704) * 1.1}`,
              } as React.CSSProperties} />
            ))}
          </div>
          <div className="final-content">
            <div className="final-orbit" aria-hidden="true"><span /><span /><span /></div>
            <div className="final-mini-line">ONE LAST THING</div>
            <div className="final-happy-word">HAPPY</div>
            <div className="final-age-lockup" aria-label="Happy 19th birthday Uzma">
              <div className="final-age-number">19</div>
              <div className="final-age-th">TH</div>
            </div>
            <div className="final-nameplate">UZMA SADAIN</div>
            <div className="final-date-line"><span /> SEPTEMBER 14 <span /></div>
            <div className="final-message">
              Here’s to the version of you
              you haven’t met yet.
              <br /><br />
              More laughter. More courage.
              More nights you never want to end.
              More memories worth keeping.
            </div>
            <div className="final-ikyk">and yes… I still wish to see you become the next maki 🌚🫠 <span>(ikyk) haha</span> ♡</div>
            <div className="final-end">MAKE THIS YEAR UNFORGETTABLE</div>
          </div>
          {finalBurst && (
      ...
      <style jsx global>{`
        ...
        * {
        ...
        .grand-intro::after {
          ...
          border-radius: 50%;
          background: radial-gradient(circle, rgba(255,248,211,.42) 0 9%, rgba(255,191,111,.20) 24%, rgba(255,128,102,.08) 42%, transparent 68%);
          filter: blur(8px);
          mix-blend-mode: screen;
          animation: introSunPulse 4.5s ease-in-out infinite;
          pointer-events: none;
        }
        ...
        .grand-intro::before {
          ...
          inset: 0;
          background:
            radial-gradient(circle at 50% 44%, rgba(255,185,145,.16), transparent 20%),
            linear-gradient(180deg, rgba(255,186,145,.08), transparent 38%, rgba(0,0,0,.18));
          pointer-events: none;
          z-index: 1;
        }
        ...
        .intro-orbit-one { animation: introOrbitA 11s linear infinite; }
        .intro-orbit-two {
          width: min(50vw, 640px);
          height: min(50vw, 640px);
          opacity: .55;
          transform: translate(-50%, -50%) rotateX(70deg) rotateZ(-18deg);
          animation: introOrbitB 8s linear infinite reverse;
        }
        ...
        /* Advanced editorial birthday treatment */
        .advanced-birthday-wordmark {
          position: relative;
          gap: 5px;
          margin-bottom: 12px;
          text-align: left;
        ...
        .advanced-birthday-wordmark::after {
          content: "";
          position: absolute;
          left: 0;
          bottom: -10px;
          ...
          animation: editorialLine 1.6s .7s cubic-bezier(.16,1,.3,1) both;
        }

        .advanced-birthday-wordmark span:first-child {
        ...
        .advanced-birthday-wordmark span:last-child {
          font-size: clamp(50px, 9.6vw, 142px);
          line-height: .82;
          letter-spacing: -.07em;
          font-weight: 500;
          color: #fff;
          ...
          position: relative;
          text-shadow: 0 10px 45px rgba(0,0,0,.22);
        }
        ...
        .advanced-age-wrap .intro-age {
          font-size: 128px;
          font-weight: 300;
          letter-spacing: -.1em;
          color: rgba(255,255,255,.05);
          -webkit-text-stroke: 1px rgba(255,255,255,.28);
          text-shadow: 0 0 90px rgba(255,181,130,.12);
        }

        .advanced-age-wrap .intro-age-label {
          letter-spacing: .55em;
          font-size: 7px;
        }
        ...
        .birthday-wordmark {
          ...
          animation: birthdayReveal 1.05s .18s cubic-bezier(.16,1,.3,1) both;
        }
        ...
        .support-orbit-one {
          ...
          animation: supportOrbitA 13s linear infinite;
        }
        ...
        .support-orbit-two {
          ...
          animation: supportOrbitB 17s linear infinite reverse;
        }

        @keyframes supportOrbitA {
          to { transform: rotate(382deg) scaleY(.42); }
        }

        @keyframes supportOrbitB {
        ...
        .support-wish {
          ...
          overflow: hidden;
          transition: padding .45s cubic-bezier(.16,1,.3,1), border-color .45s ease;
        }
        ...
        .support-wish::before {
          ...
          width: 0;
          height: 1px;
          background: linear-gradient(90deg, rgba(255,255,255,.9), rgba(182,152,255,.15), transparent);
          transition: width .7s cubic-bezier(.16,1,.3,1);
        }

        .support-wish:hover::before { width: 100%; }
        .support-wish:hover { padding-left: 14px; border-color: rgba(255,255,255,.18); }
        ...
        .support-wish:hover .support-wish-copy p {
          opacity: 1;
        ...
        .support-wish:hover .support-wish-mark {
          opacity: .9;
        ...
        .support-wish-final {
          ...
          padding-top: 38px;
          padding-bottom: 38px;
          border-top-color: rgba(194,170,255,.20);
        }

        .support-wish-final::after {
          content: "";
          position: absolute;
          width: 180px;
          height: 180px;
          ...
          border-radius: 50%;
          background: radial-gradient(circle, rgba(180,150,255,.12), transparent 68%);
          filter: blur(10px);
          pointer-events: none;
          animation: supportFinalGlow 2.8s ease-in-out infinite;
        }

        .support-wish-final .support-wish-copy span {
          color: #d8caff;
          opacity: .92;
        }

        .support-wish-final .support-wish-copy p {
          font-size: clamp(19px, 2.35vw, 28px);
          max-width: 760px;
          opacity: .94;
        }

        .support-wish-final .support-wish-mark {
          color: #d8caff;
          opacity: .9;
        }

        @keyframes supportFinalGlow {
          50% { transform: translateY(-50%) scale(1.14); opacity: .72; }
        }
        ...
        .wish-sent-panel {
          ...
          text-align: center;
          isolation: isolate;
          animation: wishSentIn 1s cubic-bezier(.16,1,.3,1) both;
        }

        .wish-sent-panel::before {
          content: "";
          position: absolute;
          width: min(680px, 90vw);
          height: min(460px, 58vh);
          border-radius: 50%;
          background: radial-gradient(circle, rgba(255,177,91,.19) 0%, rgba(255,101,71,.09) 30%, rgba(255,70,158,.035) 52%, transparent 74%);
          filter: blur(24px);
          z-index: -2;
          animation: wishFireGlow 2.2s ease-in-out infinite;
        }
        ...
        .wish-fire-orbit span {
          ...
          margin: -12.5px -2px;
          border-radius: 999px;
          background: linear-gradient(to top, rgba(255,111,62,.05), #ff9a58 48%, #fff4cf);
          box-shadow: 0 0 10px rgba(255,161,85,.92), 0 0 25px rgba(255,98,69,.46);
          transform: rotate(var(--fire-angle)) translateY(-76px) scale(.18);
          transform-origin: center;
          opacity: 0;
          animation: wishFireBurst 1.4s var(--fire-delay) cubic-bezier(.16,1,.3,1) forwards;
        }
        ...
        .wish-fire-core {
          ...
          border-radius: 50%;
          transform: translate(-50%, -50%);
          background: radial-gradient(circle, #fffbe6 0 9%, #ffd57c 28%, rgba(255,118,75,.52) 49%, transparent 72%);
          box-shadow: 0 0 30px rgba(255,214,132,.88), 0 0 80px rgba(255,114,82,.42);
          animation: wishFireCore 1.8s ease-in-out infinite;
        }

        .wish-sent-kicker {
          font-size: 10px;
          letter-spacing: .44em;
          color: rgba(255,222,157,.92);
          margin-bottom: 12px;
        }
        ...
        .wish-sent-message {
          ...
          margin-top: 18px;
          padding: 28px 30px;
          border: 1px solid rgba(255,200,128,.22);
          background: linear-gradient(180deg, rgba(255,177,90,.06), rgba(255,255,255,.022));
          box-shadow: inset 0 0 45px rgba(255,154,86,.035), 0 22px 90px rgba(255,104,62,.09);
          font-size: clamp(19px, 2.3vw, 30px);
          line-height: 1.45;
          color: rgba(255,250,241,.97);
          text-shadow: 0 0 24px rgba(255,181,110,.2);
          animation: wishMessageReveal 1.15s .16s cubic-bezier(.16,1,.3,1) both;
        }
        ...
        .wish-my-wish {
          ...
          margin-top: 30px;
          text-align: center;
          animation: wishMessageReveal 1s .38s cubic-bezier(.16,1,.3,1) both;
        }

        .wish-my-wish span {
          font-size: 8px;
          letter-spacing: .38em;
          color: rgba(255,220,153,.62);
        }
        ...
        .wish-sent-foot {
          ...
          letter-spacing: .18em;
          opacity: .28;
          animation: wishMessageReveal 1s .54s ease both;
        }

        @keyframes wishSentIn {
          from { opacity: 0; transform: scale(.96); filter: blur(10px); }
          to { opacity: 1; transform: scale(1); filter: blur(0); }
        }

        @keyframes wishFireBurst {
          0% { opacity: 0; transform: rotate(var(--fire-angle)) translateY(-22px) scale(.16); }
          18% { opacity: 1; }
          100% { opacity: 0; transform: rotate(var(--fire-angle)) translateY(-94px) scale(1); }
        }

        @keyframes wishFireCore {
          0%,100% { transform: translate(-50%, -50%) scale(.84); opacity: .72; }
          50% { transform: translate(-50%, -50%) scale(1.18); opacity: 1; }
        }

        @keyframes wishFireGlow {
          0%,100% { transform: scale(.92); opacity: .65; }
          50% { transform: scale(1.05); opacity: 1; }
        }

        @keyframes wishMessageReveal {
        ...
        /* =================================================
           BIRTHDAY CAKE CELEBRATION — LUXURY / CINEMATIC
        ================================================= */
        .cake-celebration-slide {
          position: relative;
          min-height: 180vh;
          ...
          text-align: center;
          background:
            radial-gradient(circle at 50% 42%, rgba(255, 172, 120, .06), transparent 24%),
            radial-gradient(circle at 50% 66%, rgba(118, 74, 120, .10), transparent 34%),
            #030307;
        }
        .cake-bg-word {
          position:absolute;
        ...
        .cake-entered .cake-bg-word { transform:translate(-50%,-50%) scale(1.05); opacity:.72; }
        ...
        .cake-piece {
          ...
          height:58px;
          border-radius:10px;
          background:linear-gradient(180deg,#2b2730 0%,#15131a 55%,#09090d 100%);
        ...
        .cake-piece::before {
          ...
          inset:0;
          border-radius:inherit;
          background:linear-gradient(90deg, transparent, rgba(255,190,156,.10), transparent);
        }
        .piece-one { --fx:-270px; --fy:-150px; --fr:-24deg; }
        .piece-two { --fx:260px; --fy:-170px; --fr:20deg; }
        .piece-three { --fx:-300px; --fy:70px; --fr:-14deg; }
        .piece-four { --fx:290px; --fy:78px; --fr:15deg; }
        .piece-five { --fx:-180px; --fy:190px; --fr:-9deg; }
        .piece-six { --fx:180px; --fy:195px; --fr:8deg; }
        ...
        @keyframes cakePieceAssemble {
          0% { opacity:0; transform:translate(calc(-50% + var(--fx)), calc(-50% + var(--fy))) scale(.62) rotate(var(--fr)); }
          22% { opacity:1; }
        ...
        .cake-assembled {
          ...
          height:min(58vw,375px);
          transform:translate(-50%,-44%) scale(.76) rotateX(8deg);
          opacity:0;
        ...
        @keyframes cakeBuild {
          0% { opacity:0; transform:translate(-50%,-44%) scale(.70) rotateX(14deg) translateY(46px); }
          65% { opacity:1; transform:translate(-50%,-44%) scale(1.02) rotateX(6deg) translateY(-2px); }
          100% { opacity:1; transform:translate(-50%,-44%) scale(1) rotateX(4deg) translateY(0); }
        }
        ...
        .cake-layer {
          ...
          overflow:hidden;
          border-radius:20px;
          background:linear-gradient(180deg,#312b34 0%,#18151c 58%,#0c0b10 100%);
        ...
        .cake-layer::before {
          content:"";
          position:absolute;
          left:6%; right:6%; top:12px;
          height:1px;
          background:linear-gradient(90deg,transparent,rgba(255,215,185,.58),transparent);
          opacity:.72;
        }
        .cake-layer::after {
          content:"";
          position:absolute;
          inset:0;
          background:linear-gradient(120deg,rgba(255,255,255,.08),transparent 26%,transparent 72%,rgba(214,164,255,.04));
          pointer-events:none;
        }
        .cake-layer-top { top:11%; width:56%; height:22%; }
        .cake-layer-mid { top:31%; width:75%; height:26%; }
        .cake-layer-bottom { top:54%; width:91%; height:29%; }
        ...
        .cake-frosting {
          ...
          height:7px;
          border-radius:999px;
          background:linear-gradient(90deg,transparent 0%,#f7e7da 16%,#fff4eb 50%,#f7e7da 84%,transparent 100%);
          box-shadow:0 0 18px rgba(255,226,204,.16), 0 0 4px rgba(255,255,255,.24);
        }
        .frosting-one { top:37%; width:68%; }
        .frosting-two { top:61%; width:84%; }
        ...
        .candle-body {
          ...
          height:76px;
          border-radius:6px;
          background:linear-gradient(90deg,#7d6a62 0%,#e9d7cd 35%,#fff3e8 53%,#8d776f 100%);
          border:1px solid rgba(255,255,255,.18);
          box-shadow:0 0 16px rgba(255,222,202,.13), inset 0 0 10px rgba(255,255,255,.18);
        }
        .candle-flame {
          position:absolute;
          left:50%; top:-22px;
          width:19px; height:33px;
          transform:translate3d(-50%, var(--candle-sway,0px), 0) rotate(var(--candle-bend,0deg)) scale(calc(.68 * var(--candle-scale,1)));
          border-radius:60% 40% 60% 40%;
          background:radial-gradient(circle at 50% 72%,#fff 0 17%,#ffd79e 30%,#ff9b6e 56%,#ff6c8f 72%,transparent 73%);
          filter:drop-shadow(0 0 15px rgba(255,171,120,.72));
          opacity:.34;
          animation:candleFlicker 1s ease-in-out infinite;
        }
        @keyframes candleFlicker {
          0%,100% { transform:translate3d(-50%, var(--candle-sway,0px), 0) rotate(var(--candle-bend,0deg)) scale(calc(var(--candle-scale,1) * .90)); }
          50% { transform:translate3d(-50%, var(--candle-sway,0px), 0) rotate(calc(var(--candle-bend,0deg) - 4deg)) scale(calc(var(--candle-scale,1) * 1.06)); }
        }
        .cake-top-glow {
          position:absolute;
          left:50%; top:-17%;
          ...
          transform:translateX(-50%);
          border-radius:50%;
          background:radial-gradient(circle,rgba(255,203,177,.12),rgba(255,131,111,.04) 44%,transparent 72%);
          filter:blur(18px);
        }
        .cake-shadow {
        ...
        .light-cake-button {
          ...
          padding:15px 23px;
          border-radius:999px;
          border:1px solid rgba(255,225,202,.18);
          ...
          letter-spacing:.24em;
          cursor:pointer;
          box-shadow:inset 0 1px 0 rgba(255,255,255,.08),0 10px 35px rgba(0,0,0,.18);
        ...
        .light-cake-button:not(:disabled):hover { transform:translateY(-2px) scale(1.02); background:rgba(255,255,255,.07); box-shadow:0 0 34px rgba(255,181,143,.12); }
        @keyframes lightPulse { 50%{box-shadow:0 0 34px rgba(255,181,143,.10);} }

        .cake-sky-celebration { position:absolute; inset:0; z-index:16; background:radial-gradient(circle at center,rgba(91,61,179,.18),transparent 42%),linear-gradient(180deg,rgba(3,2,10,.16),rgba(3,2,10,.94)); animation:skyReveal 1.4s ease forwards; }
        @keyframes skyReveal { from{opacity:0;transform:scale(1.05)} to{opacity:1;transform:scale(1)} }
        .cake-celebration-wash { position:absolute; inset:0; background:radial-gradient(circle at center,rgba(255,235,219,.08),transparent 22%); animation:washPulse 3.4s ease-in-out infinite; }
        @keyframes washPulse { 50%{opacity:.5;transform:scale(1.12)} }
        .cake-celebration-title { position:absolute; left:50%; top:24%; transform:translate(-50%,-50%); z-index:40; text-align:center; width:92%; animation:bdayTitleIn 1.8s cubic-bezier(.16,1,.3,1) .3s both; text-shadow:0 0 28px rgba(255,196,165,.12); }
        ...
        .cake-balloon {
          --balloon-x:50%; --balloon-delay:0s; --balloon-scale:1; --balloon-drift:0px;
          position:absolute; left:var(--balloon-x); top:-90px; width:34px; height:46px; border-radius:50% 50% 47% 47%; opacity:0;
        ...
        .cake-balloon::before { content:""; position:absolute; inset:0; border-radius:inherit; background:radial-gradient(circle at 30% 24%,rgba(255,255,255,.72) 0 7%,rgba(255,255,255,.18) 8% 19%,transparent 20%),linear-gradient(145deg,rgba(255,255,255,.16),rgba(255,255,255,0) 36%); border:1px solid rgba(255,255,255,.2); box-shadow:inset -6px -8px 14px rgba(0,0,0,.12),inset 5px 5px 12px rgba(255,255,255,.12),0 0 18px rgba(255,255,255,.07); }
        .cake-balloon::after { content:""; position:absolute; left:50%; bottom:-5px; width:8px; height:9px; transform:translateX(-50%) rotate(45deg); border-radius:2px; background:inherit; }
        .cake-balloon i { position:absolute; left:50%; top:calc(100% + 3px); width:1px; height:76px; transform-origin:top center; background:linear-gradient(180deg,rgba(255,255,255,.62),rgba(255,255,255,.08)); box-shadow:0 0 4px rgba(255,255,255,.16); }
        ...
        @keyframes balloonFall {
          0%{opacity:0;transform:translate3d(calc(-50% + var(--balloon-drift) + var(--pointer-drift-x,0px)),calc(-40px + var(--pointer-drift-y,0px)),0) rotate(calc(-8deg + var(--pointer-rot,0deg))) scale(var(--balloon-scale));}
          8%{opacity:.96;}
          42%{opacity:.96;transform:translate3d(calc(-50% + var(--balloon-drift) * -.35 + var(--pointer-drift-x,0px)),calc(48vh + var(--pointer-drift-y,0px)),0) rotate(calc(7deg + var(--pointer-rot,0deg))) scale(var(--balloon-scale));}
          72%{opacity:.9;transform:translate3d(calc(-50% + var(--balloon-drift) * .55 + var(--pointer-drift-x,0px)),calc(92vh + var(--pointer-drift-y,0px)),0) rotate(calc(-5deg + var(--pointer-rot,0deg))) scale(var(--balloon-scale));}
          100%{opacity:0;transform:translate3d(calc(-50% + var(--balloon-drift) + var(--pointer-drift-x,0px)),calc(118vh + var(--pointer-drift-y,0px)),0) rotate(calc(9deg + var(--pointer-rot,0deg))) scale(calc(var(--balloon-scale) * .94));}
        }
        ...
        .cake-lit .cake-light-ui { opacity:0; pointer-events:none; transition:opacity .8s ease; }
        .cake-lit .cake-assembled { transform:translate(-50%,-44%) scale(1.14) rotateX(4deg); filter:drop-shadow(0 0 36px rgba(255,186,151,.16)); }
        .cake-lit .cake-top-glow { width:260px; height:260px; top:-30%; background:radial-gradient(circle,rgba(255,226,198,.30) 0%,rgba(255,151,117,.14) 30%,rgba(255,113,174,.035) 58%,transparent 73%); filter:blur(12px); animation:cakeLightBloom 1.6s ease-out forwards,cakeLightPulse 2.4s 1.6s ease-in-out infinite; }
        .cake-lit .candle-flame { opacity:1; transform:translate3d(-50%, var(--candle-sway,0px), 0) rotate(var(--candle-bend,0deg)) scale(var(--candle-scale,1)); animation:candleFlicker .75s ease-in-out infinite,flameGlow 1.2s ease-in-out infinite; }
        .cake-lit .candle-body { box-shadow:0 0 16px rgba(255,225,249,.25); }
        .cake-lit .cake-shadow { box-shadow:0 0 35px rgba(255,134,214,.13); }
        ...
        @keyframes flameGlow { 50%{filter:drop-shadow(0 0 24px rgba(255,178,133,.92));} }
        ...
        @media (max-width: 768px) {
          ...
          .cake-piece { width:82px; height:48px; }
          ...
          .cake-lit .cake-assembled { transform:translate(-50%,-44%) scale(1.08) rotateX(3deg); }
        ...
        /* =================================================
           FINAL — CINEMATIC AGE REVEAL OVERRIDES
        ================================================= */
        .final-slide {
          isolation: isolate;
          min-height: 135vh;
          background:
            radial-gradient(circle at 50% 42%, rgba(168,143,255,.14), transparent 23%),
            radial-gradient(circle at 50% 64%, rgba(255,199,236,.055), transparent 28%),
            #05050a;
        }
        .final-slide::after {
          content: "";
          ...
          inset: 0;
          pointer-events: none;
          background: radial-gradient(circle at 50% 47%, transparent 24%, rgba(0,0,0,.72) 100%);
          z-index: 3;
        }
        .final-vignette {
          position:absolute; inset:-10%; z-index:2; pointer-events:none;
          background:radial-gradient(circle at 50% 45%, transparent 28%, rgba(0,0,0,.34) 68%, rgba(0,0,0,.78) 100%);
        }
        .final-content {
        ...
        .final-age-lockup {
          position:relative;
          margin-top:-4px;
          min-height:clamp(180px,31vw,365px);
          display:flex;
          align-items:flex-end;
          justify-content:center;
        }
        .final-age-number {
          ...
          text-shadow:
            0 0 22px rgba(195,176,255,.12),
            0 0 70px rgba(195,176,255,.06);
          position:relative;
          z-index:2;
          animation:final19Breathe 5s ease-in-out infinite;
        }
        .final-age-number::after {
          content:"19";
          position:absolute;
          inset:0;
        ...
        .final-date-line span {
          width:28px; height:1px; background:rgba(255,255,255,.2); display:block;
        }
        .final-message {
          margin:74px auto 0;
          max-width:560px;
        ...
        .final-orbit {
          width:min(78vw,740px); height:min(78vw,740px);
          top:50%; left:50%;
        ...
        .final-orbit span {
          border-color:rgba(197,180,255,.15) !important;
          box-shadow:0 0 18px rgba(197,180,255,.035);
        }
        .final-spark-field {
        ...
        .final-spark-field i {
          ...
          opacity:0;
          animation:finalSpark 3.8s var(--sd) ease-in-out infinite;
          transform:scale(var(--ss));
        }
        .final-firework-rings { z-index:1 !important; opacity:.72; }
        .final-firework-rings span { border-color:rgba(204,188,255,.16) !important; }
        @keyframes final19Breathe {
          0%,100% { transform:translateY(0) scale(1); filter:drop-shadow(0 0 0 rgba(180,160,255,0)); }
          50% { transform:translateY(-7px) scale(1.012); filter:drop-shadow(0 0 26px rgba(180,160,255,.11)); }
        }
        @keyframes finalSpark {
          0%,100% { opacity:0; transform:scale(.5); }
          18% { opacity:.15; }
          46% { opacity:.9; transform:scale(1.2); }
          70% { opacity:.25; }
        }
...
.memory-bg-image {
  ...
  overflow: hidden;
  pointer-events: none;
  opacity: var(--memory-bg-opacity, .01);
  transform: translate3d(var(--memory-bg-x, 0px), var(--memory-bg-y, 46px), 0) scale(var(--memory-bg-scale, 1.12));
  filter: blur(var(--memory-bg-blur, 8px));
  will-change: transform, opacity, filter;
  mix-blend-mode: screen;
}

.memory-bg-image::before {
  content: "";
  position: absolute;
  inset: 0;
  z-index: 1;
  background:
    radial-gradient(circle at 50% 44%, rgba(155,134,255,.18), transparent 42%),
    linear-gradient(180deg, rgba(7,6,18,.62) 0%, rgba(7,6,18,.18) 42%, rgba(7,6,18,.78) 100%);
}

.memory-bg-image::after {
  content: "";
  position: absolute;
  inset: 0;
  z-index: 2;
  background: radial-gradient(circle at center, transparent 28%, rgba(6,5,15,.72) 82%);
}
...
/* =========================================================
   ADVANCED CINEMA HOOKS
========================================================= */
.advanced-motion-enabled .memory-bg-image img {
  transform: scale(calc(1.02 + var(--advanced-photo-zoom, 0)));
  transition: transform .18s linear;
}
.advanced-motion-enabled .memory-stage {
  transform: translate3d(var(--glass-drift-x,0px), var(--glass-drift-y,0px), 0) rotateX(var(--glass-tilt-x,0deg)) rotateY(var(--glass-tilt-y,0deg));
  transition: transform .18s linear;
}
.advanced-motion-enabled .cake-stage {
  --candle-bend: 0deg;
}
.advanced-motion-enabled .candle-flame {
  transform: translateX(-50%) rotate(var(--candle-bend,0deg)) scale(var(--candle-scale,1));
}

/* =========================================================
   IGLOO-STYLE SCROLL CINEMA
   Scale-through, depth separation and spotlight transitions.
========================================================= */
.experience > .hero-slide,
.experience > .chapter-slide,
.experience > .memory-slide,
.experience > .wishes-slide,
.experience > .interactive-wish-slide,
.experience > .sky-slide,
.experience > .support-slide,
.experience > .message-slide,
.experience > .final-slide,
.experience > .cake-celebration-slide,
.experience > .maki-slide,
.experience > .letter-slide {
  --igloo-scale: .96;
  --igloo-y: 24px;
  --igloo-rotate: 0deg;
  --igloo-blur: 1px;
  ...
  --igloo-focus: .5;
  --igloo-depth: 20px;
  transform: translate3d(0,var(--igloo-y),0) rotate(var(--igloo-rotate)) scale(var(--igloo-scale));
  transform-origin: 50% 50%;
  filter: blur(var(--igloo-blur));
  opacity: var(--igloo-opacity);
  will-change: transform, filter, opacity;
  transition: filter .2s linear, opacity .2s linear;
}
.experience > .hero-slide::after,
.experience > .chapter-slide::after,
.experience > .wishes-slide::after,
.experience > .interactive-wish-slide::after,
.experience > .sky-slide::after,
.experience > .support-slide::after,
.experience > .message-slide::after,
.experience > .final-slide::after,
.experience > .cake-celebration-slide::after,
.experience > .maki-slide::after,
.experience > .letter-slide::after {
  content:"";
  position:absolute;
  inset:-8%;
  pointer-events:none;
  background:radial-gradient(circle at 50% 45%, rgba(255,255,255,calc(var(--igloo-focus) * .035)), transparent 56%);
  mix-blend-mode:screen;
  opacity:.8;
}
.experience > .hero-slide .slide-inner,
.experience > .chapter-slide .slide-inner,
.experience > .memory-slide .memory-sequence,
.experience > .maki-slide .maki-header,
.experience > .letter-slide .letter-header {
  transform: translate3d(0,var(--igloo-inner-y,0px),var(--igloo-depth,0px));
  will-change: transform;
}
.experience > .memory-slide .memory-stage,
.experience > .cake-celebration-slide .cake-stage,
.experience > .cake-celebration-slide .cake-sky-celebration,
.experience > .maki-slide .maki-reveal,
.experience > .letter-slide .letter-paper-wrap {
  transform: translate3d(0,var(--igloo-inner-y,0px),0) scale(var(--igloo-scale-inner,1));
  will-change: transform;
}
.experience > .memory-slide .memory-bg-word,
.experience > .hero-slide .hero-bg-word,
.experience > .chapter-slide .chapter-bg-word,
.experience > .wishes-slide .wishes-bg-word,
.experience > .sky-slide .sky-bg-word,
.experience > .support-slide .support-bg-word,
.experience > .message-slide .message-bg-word,
.experience > .final-slide .final-bg-word,
.experience > .maki-slide .maki-bg-word,
.experience > .letter-slide .letter-bg-word {
  transform: translate3d(calc(var(--ambient-shift, 0px) * .35), calc(var(--igloo-depth) * -0.22), 0) scale(calc(.92 + var(--igloo-focus) * .08));
  transition: transform .18s linear;
}
.experience > .memory-slide .photo-card {
  box-shadow: 0 38px 120px rgba(0,0,0,.40), 0 0 calc(35px + var(--igloo-focus) * 55px) rgba(155,134,255,calc(.03 + var(--igloo-focus) * .11));
}
@media (max-width:768px) {
  .experience > .hero-slide,
  .experience > .chapter-slide,
...
/* =========================================================
   DAY → SUNSET → NIGHT WORLD
   Scroll-driven atmospheric background.
========================================================= */
.day-night-world {
  position: fixed;
  inset: 0;
...
.day-night-sky {
  ...
  filter:
    saturate(calc(1.04 - var(--dn-night) * .18))
    brightness(calc(1 - var(--dn-night) * .66));
...
.day-night-horizon {
  ...
  background: radial-gradient(ellipse at center,
    rgba(255,218,163, calc(.34 * var(--dn-sunset) + .06 * (1 - var(--dn-night)))),
    rgba(255,161,120, calc(.18 * var(--dn-sunset))),
    transparent 68%
  );
  filter: blur(24px);
  opacity: calc(.45 + var(--dn-sunset) * .45);
  transform: translateY(calc(var(--dn-scroll, 0) * -2px));
}
.day-night-sun {
  position: absolute;
  ...
  background: radial-gradient(circle at 50% 46%, #fffdf0 0 18%, #ffe6a8 42%, #ffb763 66%, rgba(255,166,88,.08) 74%, transparent 75%);
  filter: blur(.2px) drop-shadow(0 0 55px rgba(255,207,115,.42));
  opacity: calc(1 - var(--dn-night) * 1.05);
  transform: translate3d(calc(var(--dn-sun-x) * 1px), calc(var(--dn-sun-y) * 1px), 0) scale(calc(.9 + var(--dn-sun-scale) * .28));
}
.day-night-sun-halo {
  position: absolute;
  inset: -55%;
  border-radius: 50%;
  background: radial-gradient(circle, rgba(255,233,181,.28), transparent 64%);
  filter: blur(22px);
}
.day-night-moon {
  position: absolute;
  ...
  background: radial-gradient(circle at 36% 32%, #fffef5 0 20%, #f8f2dc 48%, #eee5c6 78%, #e3d8b6 100%);
  box-shadow: 0 0 36px rgba(255,248,210,.42), 0 0 100px rgba(255,244,195,.22);
  opacity: var(--dn-night);
  will-change: transform, opacity;
  transform: translate3d(calc(var(--dn-moon-x) * 1px), calc(var(--dn-moon-y) * 1px), 0);
}
.day-night-stars {
  position: absolute;
  inset: -16vh -10vw;
  z-index: 4;
  opacity: var(--dn-stars);
  transform: translate3d(0, calc(var(--dn-scroll, 0) * -0.055px), 0) scale(1.04);
  transition: opacity .25s linear;
  will-change: transform, opacity;
}
.day-night-stars span {
  position: absolute;
  width: var(--star-size);
  height: var(--star-size);
  border-radius: 50%;
  background: #fff;
  box-shadow: 0 0 4px rgba(255,255,255,.72), 0 0 10px rgba(190,210,255,.38);
  animation: dayNightTwinkle var(--star-duration) ease-in-out var(--star-delay) infinite;
}
.day-night-clouds {
  position: absolute;
  inset: 0;
  opacity: calc((1 - var(--dn-night)) * .42);
}
.day-night-clouds::before, .day-night-clouds::after {
  content: "";
  position: absolute;
  border-radius: 50%;
  filter: blur(18px);
...
.day-night-clouds::before { width: 34vw; height: 7vw; left: 5%; top: 22%; transform: translateX(calc(var(--dn-scroll,0) * .08px)); }
.day-night-clouds::after { width: 28vw; height: 6vw; right: 2%; top: 34%; transform: translateX(calc(var(--dn-scroll,0) * -.06px)); }
.day-night-twilight {
  position: absolute;
  inset: 0;
  background:
    radial-gradient(ellipse at 50% 64%, rgba(255,210,153,.34) 0%, rgba(255,135,92,.18) 30%, transparent 62%),
    linear-gradient(180deg, rgba(255,195,124,.03) 0%, rgba(221,78,70,.22) 48%, rgba(83,34,58,.62) 100%);
  opacity: calc((1 - var(--dn-night)) * (.48 + var(--dn-sunset) * .48));
}
.day-night-nightwash {
  position: absolute;
  inset: 0;
...
@keyframes dayNightTwinkle { 0%,100% { opacity:.25 } 50% { opacity:1 } }
@media (max-width:768px) {
  .day-night-clouds { opacity: calc((1 - var(--dn-night)) * .25); }
  .day-night-stars { transform: translate3d(0, calc(var(--dn-scroll,0) * -.09px), 0); }
}

/* =========================================================
   EXTRA CINEMATIC FX
   ========================================================= */
.cinematic-fx { position: fixed; inset: 0; z-index: 120; pointer-events: none; overflow: hidden; }
.fx-vignette { position: absolute; inset: 0; background: radial-gradient(circle at center, transparent 42%, rgba(0,0,0,.18) 78%, rgba(0,0,0,.36) 100%); }
.fx-scan { position: absolute; left: 0; top: -12vh; width: 100%; height: 1px; background: linear-gradient(90deg, transparent, rgba(255,255,255,.10), transparent); opacity: .16; transform: translateY(var(--fx-scroll-shift,0px)) skewY(var(--fx-scroll-skew,0deg)); transition: transform .12s linear; }
.light-cake-button, .firework-button, .maki-lock, .letter-envelope, .letter-close, .maki-reset, .wish-choice, .wish-seal-button { transform: translate3d(var(--mag-x,0px), var(--mag-y,0px), 0); }
@media (max-width:768px) { .fx-vignette { background: radial-gradient(circle at center, transparent 44%, rgba(0,0,0,.2) 100%); } }
@media (prefers-reduced-motion: reduce) { .cinematic-fx { display:none; } }
      `}
</style>

<style jsx global>{`
  .advanced-motion-enabled .light-cake-button,
  .advanced-motion-enabled .firework-button,
  .advanced-motion-enabled .maki-lock,
  .advanced-motion-enabled .letter-envelope,
  .advanced-motion-enabled .letter-close,
  .advanced-motion-enabled .maki-reset,
  .advanced-motion-enabled .wish-choice,
  .advanced-motion-enabled .wish-seal-button {
    --mag-x: 0px;
    --mag-y: 0px;
    transform: translate3d(var(--mag-x), var(--mag-y), 0);
  }
  .advanced-motion-enabled .photo-card { transform-style: preserve-3d; }
`}</style>

<style jsx global>{`
  .advanced-motion-enabled .experience > .memory-slide .memory-stage {
    transform: translate3d(
      ...
      0
        ...
        @media (max-width: 800px) {
          .final-content { padding:12vh 0 8vh; }
          .final-happy-word { font-size:clamp(26px,8vw,42px); letter-spacing:.24em; margin-left:.24em; }
          .final-age-lockup { min-height:210px; }
          .final-age-number { font-size:clamp(170px,55vw,270px); }
          .final-age-th { right:3%; bottom:20%; font-size:10px; }
          .final-nameplate { font-size:11px; letter-spacing:.28em; margin-left:.28em; }
          .final-date-line { font-size:7px; letter-spacing:.28em; margin-left:.28em; gap:9px; }
          .final-date-line span { width:16px; }
          .final-message { margin-top:52px !important; max-width:320px; font-size:13px !important; }
          .final-ikyk { max-width:320px; font-size:11px !important; }
          .final-end { margin-top:62px !important; font-size:7px !important; letter-spacing:.28em !important; margin-left:.28em; }
        }
`}
</style>
    </main>
